...
**Scope: this boundary bounds decisions written through the DECISION-ENVELOPE path** — the Aegis governance ingress (`/v1/govern` → the canonical envelope builder). Records written through that path are, from the cutover onward, signed under **`decision_envelope_v2`**. A v2 envelope commits to the **complete, ordered effective policy set** that governed the decision via `policy_set_hash` (SHA-256 of the canonical set — global guards in their governed order, then adapter rules; evaluation is first-match, so **order is part of the commitment**), while keeping the full policy **definition private**.

**The ledger also holds records that are NOT decision envelopes at all**, and a reader querying it will find post-cutover rows that are neither `decision_envelope_v1` nor `v2` — that is not a breach of this boundary. A separate write path (the trading path) builds its own inner payload (`action_type = TRADING_GOVERNANCE_DECISION`) and hashes it directly; those records carry **no `schema_version`** and never enter the decision-envelope construction. The v1→v2 cutover says nothing about them, and they are outside its scope. The cutover claim is specifically: *from the first `decision_envelope_v2` record onward, every decision written through the decision-envelope path is v2* — not that every ledger row is.
